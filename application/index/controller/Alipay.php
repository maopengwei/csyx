...
class Alipay extends Api
{
    ...
    public function index()
    {
       ...
       $d = input('post.');
        ...
        $uu = Db::name('user')->where('id',$d['us_id'])->field('id,us_account,us_safe_pwd')->find();
        if(!$uu){
            $this->error('该用户不存在');
        }
        ...
        //     $this->e_msg('安全密码不正确');
        // }
        $orderid = "ch" . date("YmdHis") . rand(100, 999);
        $rel = model('AlipayPay')->tianjia($d['us_id'], $orderid, $d['num'], $d['type'], $d['relevance']);
        if($rel){
            $rrr = model('AlipayPay')->back_success($orderid);
            $this->s_msg('支付成功');
        }
